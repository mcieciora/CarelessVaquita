        ...
        stage ("Staging") {
            ...
            parallel {
                stage ("Push docker image") {
                    when {
                        expression {
                            return BRANCH_TO_USE == "master" || BRANCH_TO_USE == "develop" && IS_NIGHTLY == false
                        }
                    }
                    steps {
                        script {
                            docker.withRegistry("", "dockerhub_id") {
                                sh "docker build --build-arg PYTHON_BASE_IMAGE=python:${PYTHON_VERSION}-alpine --no-cache -t custom_image ."
                ...
                stage ("Push tag") {
                    when {
                        expression {
                            return BRANCH_TO_USE == "master" && IS_NIGHTLY == false
                        }
                    }
                    steps {
                        script {
