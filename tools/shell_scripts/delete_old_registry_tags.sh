...
curl -s -X GET "$REGISTRY_URL/v2/$DOCKERHUB_REPO/tags/list" > all_tags.json

jq -c '.tags[]' all_tags.json | while read -r TAG; do
  TAG_BLOB_VALUE=$(curl -s -X POST "$REGISTRY_URL/v2/$DOCKERHUB_REPO/manifests/$TAG" -H "Accept: application/vnd.docker.distribution.manifest.v2+json" | jq .config.digest)
  echo "TAG_BLOB_VALUE: $TAG_BLOB_VALUE"
  TAG_CREATED=$(curl -s "$REGISTRY_URL/v2/$DOCKERHUB_REPO/blobs/sha256:$TAG_BLOB_VALUE | jq -r .created")
  ...
  if [ $DATE_DIFF -ge "$DAYS_TO_KEEP_REGISTRY_IMAGES_POLICY" ]; then
    ...
    curl -X DELETE "$REGISTRY_URL/v2/$DOCKERHUB_REPO/manifests/$TAG_BLOB_VALUE"
